...
const PERSONAS = [
  {
    id: 1, name: "The Golden Chapter", emoji: "☀️", short: "Golden",
    tag: "Adults 55+", color: "#b8922a", light: "#fef3c7",
  ...
  {
    id: 2, name: "The Turning Point", emoji: "🌱", short: "Turning",
  ...
  {
    id: 3, name: "The New Chapter", emoji: "🌸", short: "New",
    tag: "Pregnant / Hormonal", color: "#db2777", light: "#fce7f3",
    painPoints: ["Puffy feet in 3rd trimester", "Monthly hormonal swelling cycles", "Want natural, not pharmaceutical"],
    angle: "All-botanical formula · Plant-based, no harsh chemicals",
    avatar: "Woman 28–35, visibly pregnant, bright soft light, gentle and hopeful",
    compliance: "Must include: Consult your OB or midwife before use.",
  },
  {
    id: 4, name: "On-Your-Feet Crew", emoji: "👟", short: "Workers",
    tag: "Workers / Standing Jobs", color: "#2563eb", light: "#dbeafe",
  ...
  {
    id: 5, name: "Side Effect Survivors", emoji: "💊", short: "Survivors",
    tag: "Medication-Induced Swelling", color: "#7c3aed", light: "#ede9fe",
    painPoints: ["Need the meds but hate the swelling", "Swelling started with the prescription", "Doctors don't offer solutions"],
    angle: "Natural botanical complement · Buchu + Uva Ursi support fluid balance",
    avatar: "Man or woman 45–60, direct to camera, home office, matter-of-fact",
...
const SYSTEM_PROMPT = (persona, format, type, week) => `You are an expert TikTok content strategist for Lipitrex Water Pills — a botanical edema supplement on Amazon (ASIN: B08B9SH5XH) by Harvest Vitality. Contains Dandelion Leaf, Horse Chestnut, Ginkgo Biloba, Uva Ursi, and Buchu Leaf. GMP certified, FDA registered, made in the USA. Ranked #12 in Diuretic Supplements on Amazon.
...
CONTENT RULE: Solution-forward and empowering always. No doom, no shame, no fear. Lead with empathy, land on possibility.
...
AVATAR MATCHING — Non-negotiable:
${persona.avatar}
Pacing and delivery must match this persona's energy level and life context.

Generate a complete ${type === 'video' ? 'TikTok VIDEO' : 'CAROUSEL'} post package. Use these exact headers:

## CONTENT ID
Format: LX-${type === 'video' ? 'V' : 'C'}-[###]-P${persona.id}-W${week}
Assign a sequential number. List all 15 attribute tags.

## HOOK
One line. Stops the scroll. ${type === 'video' ? 'Spoken aloud in first 2 seconds.' : 'Bold text on slide 1.'}

## ${type === 'video' ? 'SCRIPT' : 'SLIDE BREAKDOWN'}
${type === 'video' ? 'Full script: [HOOK 0-3s] [BODY] [CTA — soft/medium/hard options]' : 'Every slide: number, headline text, body text (1-2 sentences max), visual direction for AIVideo'}

## AI ${type === 'video' ? 'VIDEO' : 'IMAGE'} DIRECTION
${type === 'video' ? 'Avatar (exact demographic match), scene, setting, wardrobe, delivery pace, b-roll, text overlays, audio mood' : 'Visual style, color palette, typography direction, slide layout, brand consistency notes'}
...
## COMPLIANCE CHECK
Full checklist. Flag any issues. ${persona.id === 3 || persona.id === 5 ? '⚠️ Required disclaimer: ' + persona.compliance : 'No disclaimer required for this persona.'}

## PIPELINE NOTE
${type === 'carousel' ? 'Rate video transfer potential: High / Medium / Low — and exactly why. What would the video version look like?' : 'Rate paid ad potential: High / Medium / Low — what attribute combination drove performance?'}

## STAGE RECOMMENDATION
Based on the content attributes, recommend the likely first stage transition after organic posting: Paid / Evergreen / Cross-Platform / Repurpose candidate`;
...
export default function LipitrexDashboard() {
  ...
  const [aiVideoKey, setAiVideoKey] = useState("");
  const [aiVideoConnected, setAiVideoConnected] = useState(false);
  ...
  const generatePost = async (persona, type) => {
    ...
    const newId = `LX-${type === 'video' ? 'V' : 'C'}-${num}-P${persona.id}-W${week}`;
...
Make it specific, vivid, and warm. The viewer should feel understood before they feel sold to.`
          }]
        })
      });
      ...
      const entry = {
        ...
        attributes: {
          ...
          compliance: persona.id === 3 || persona.id === 5 ? "Required" : "None",
          week,
          platform: "TikTok",
        }
      };
        ...
        <div style={{ maxWidth: "960px", margin: "0 auto", padding: "0 20px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0 0" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div style={{ fontSize: "12px", color: T.muted }}>Week</div>
              {[1,2,3,4,5].map(w => (
                <button key={w} onClick={() => setWeek(w)} style={{
                  width: "28px", height: "28px", borderRadius: "6px",
                  border: `1px solid ${week === w ? T.gold : T.border}`,
                  background: week === w ? T.goldLight : T.white,
                  color: week === w ? T.goldDark : T.muted,
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "24px" }}>
              {[
                ...
                { label: "Week", value: week, sub: "Offset rotation active" },
              ].map(s => (
            ...
            {/* AIVideo connection */}
            <Card style={{ marginBottom: "20px", padding: "16px", borderLeft: `3px solid ${aiVideoConnected ? T.green : T.gold}` }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  <span style={{ fontSize: "20px" }}>🎥</span>
                  <div>
                    <div style={{ fontSize: "13px", fontWeight: 700 }}>AIVideo.com Direct Connection</div>
                    <div style={{ fontSize: "11px", color: T.muted }}>
                      {aiVideoConnected ? "Connected — posts generate directly to AIVideo" : "Enter API key to enable one-click video production"}
                    </div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                  {!aiVideoConnected && (
                    <Input value={aiVideoKey} onChange={setAiVideoKey} placeholder="AIVideo API key..." style={{ width: "200px" }} />
                  )}
                  <Btn variant={aiVideoConnected ? "ghost" : "primary"} onClick={() => {
                    if (aiVideoKey.length > 5) setAiVideoConnected(true);
                    else if (aiVideoConnected) { setAiVideoConnected(false); setAiVideoKey(""); }
                  }}>
                    {aiVideoConnected ? "Disconnect" : "Connect"}
                  </Btn>
                  {aiVideoConnected && <Badge color={T.green} bg={T.greenLight}>✓ Connected</Badge>}
                </div>
              </div>
            </Card>
